...
const getErrorLable = (config, value) => {
  if (config.required && !value) return config.required.message;
  if (config.max && value.length > config.max.value) return config.max.message;
  if (config.min && value.length < config.min.value) return config.min.message;
    ...
    let validate = {
      errorEmail:
        request.email != undefined
          ? getErrorLable(configAuthentication.email, request.email)
          : undefined,
      errorPassword:
        request.password != undefined
          ? getErrorLable(configAuthentication.password, request.password)
          : undefined,
      errorPhone:
        request.phone != undefined
          ? getErrorLable(configAuthentication.phone, request.phone)
          : undefined,
      errorUser:
        request.user != undefined
          ? getErrorLable(configAuthentication.username, request.user)
          : undefined,
