...
export default ({
  ...
  const renderShowCartDetail = (title, money, type) => {
    return (
      <View style={{flexDirection: 'row', marginBottom: width * 0.03}}>
        ...
        <Text
          style={{
            ...
            flex: 1,
            textAlign: 'right',
            color: type == 1 ? '#EC5858' : 'black',
          }}
        >
          {type == 1 ? '-' : ''} ${money ? money : 0}
        </Text>
      </View>
    );
  };
  const total = () => {
    let tl = eval(
      `${moneyTotal ? moneyTotal : 0} + ${shippingFee ? shippingFee : 0} + ${
        estimatingTax ? estimatingTax : 0
      } - ${discount}`,
    );
    return tl;
  };
  ...
  return (
    <>
      ...
      <View style={{flexDirection: 'row', marginVertical: width * 0.03}}>
        ...
        <Text style={{...AppStyle.style.h5, flex: 1, textAlign: 'right'}}>
          ${total()}
        </Text>
      </View>
    </>
