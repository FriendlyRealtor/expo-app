...
export default ({navigation}) => {
  ...
  const minusOrder = index =>
    setCarts(carts => {
      var cart = [...carts];
      cart[index].count =
        cart[index].count >= 1 ? cart[index].count - 1 : cart[index].count;
      return cart;
    });

  const plusOrder = index =>
    setCarts(carts => {
      var cart = [...carts];
      cart[index].count++;
      return cart;
    ...
    >
      <View
        style={[
          ...
          AppStyle.style.smallPaddingTop,
        ]}
      >
        {!carts || carts.length == 0 ? (
          <Empty onBackPress={() => navigation.goBack()} />
        ) : (
          <>
            <FlatList
              data={carts}
