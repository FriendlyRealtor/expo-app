...
export default ({card, onChangePress = () => {}}) => {
  return (
    <View style={[AppStyle.style.marginTop]}>
      <View style={[{flexDirection: 'row'}, AppStyle.style.marginBottom]}>
        <View style={{flex: 1}}>
          <Text style={[AppStyle.style.p]}>{card?.holder}</Text>
          {card?.email ? (
            <Text style={[AppStyle.style.p, {opacity: 0.5}]}>
              {card?.email}
            </Text>
          ) : null}
          {card?.phone ? (
          ...
          <View style={[AppStyle.style.paddingTop]}>
            {card?.cardNo ? (
              <View
                style={[{flexDirection: 'row'}, AppStyle.style.marginBottom]}
              >
                <Text style={{flex: 1}}>Card No</Text>
                <Text>:</Text>
                <Text style={[{flex: 2, textAlign: 'right'}, {opacity: 0.5}]}>
                  {convertCardNo(card.cardNo)}
                </Text>
              </View>
            ) : null}
            {card?.expire ? (
