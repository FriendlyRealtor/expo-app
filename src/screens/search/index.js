...
export default ({ navigation, route }) => {
    ...
    const recentSearchs = () => {
        return oldSearch.length > 0 ? <>
            <Text style={[AppStyle.style.p, AppStyle.style.marginBottomLarge, { color: AppStyle.color.black }]}>RECENT SEARCHS</Text>
            {
                oldSearch.map(value => <TouchableOpacity key={`recent-search-${value}`} activeOpacity={0.8} style={[AppStyle.style.marginLeftLarge]}><Text style={[AppStyle.style.p, AppStyle.style.marginBottom, {color: AppStyle.color.gray}]}>{value}</Text></TouchableOpacity>)
            }
        </> : <></>
    }
    ...
    return <Layout>
        <Search containerStyle={{ marginBottom: 0 }} submit={true} hiddenBottom={true} onSubmit={(value) => setInputSearch(value)} />
        <FlatList 
