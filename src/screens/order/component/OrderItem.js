...
const style = StyleSheet.create({
    widgetContainer: { position: "absolute", top: 0, bottom: 0, left: 0, right: 0, backgroundColor: AppStyle.color.brown + '80', borderRadius: 10 },
    widgetIcon: { width: 25, height: 25, alignSelf: "center", resizeMode: "contain" }
})
export default ({ item, index }) => {
    ...
    return <Product
        ...
        imageChild={() => <Animated.View style={[styleWidget, style.widgetContainer, AppStyle.style.middle, { zIndex: showWidget ? 2 : -2 }]}>
            <TouchableOpacity onPress={selectWidgetReOrder} style={[AppStyle.style.marginBottomLarge]}>
                <Image source={icons.reOrder} style={style.widgetIcon} />
                <Text style={[AppStyle.style.p, { color: AppStyle.color.white }]}>Reorder</Text>
            </TouchableOpacity>
            <TouchableOpacity onPress={selectWidgetWriteReview}>
                <Image source={icons.writeReviewer} style={style.widgetIcon} />
                <Text style={[AppStyle.style.p, { color: AppStyle.color.white }]}>Write a Review</Text>
            </TouchableOpacity>
        </Animated.View>}
    />
}