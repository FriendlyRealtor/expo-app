...
export default ({trackOrder, ...FlatListProps}) => {
      ...
      <View style={[AppStyle.style.marginHorizontalLarge]}>
        <View style={{flexDirection: 'row', alignItems: 'center'}}>
          <Image
            ...
            style={[
              Style.iconVerify,
              {opacity: index < trackOrder.realtime.length - 1 ? 0.6 : 1},
            ]}
          />
          <View style={[{flex: 1}, AppStyle.style.marginRight]}>
            <Text style={[AppStyle.style.p, {fontWeight: 'normal'}]}>
              {item.title}
            </Text>
            <Text style={[{color: 'gray', fontSize: 13}]}>{item.subTitle}</Text>
          </View>
          {item.endTrack ? (
            <Text style={[AppStyle.style.p, {color: AppStyle.color.green}]}>
              Confirmed
            </Text>
          ) : (
            <Text>{item.time}</Text>
    ...
    () => (
      <View style={AppStyle.style.marginHorizontalLarge}>
        ...
        <Text style={[AppStyle.style.p, AppStyle.style.marginBottomLarge]}>
          Order Number:{' '}
          <Text style={{color: 'gray'}}>#{trackOrder.orderNumber}</Text>
        </Text>
        <Text style={[AppStyle.style.h5, {fontWeight: 'normal'}]}>
          ETA:{' '}
          <Text style={{color: AppStyle.color.gray}}>{trackOrder.eta}</Text>
        </Text>
      </View>
    ),
    [],
  ...
  return trackOrder ? (
    <View style={[AppStyle.style.marginBottomLarge]}>
      <FlatList
