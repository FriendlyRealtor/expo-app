...
export default ({navigation, route}) => {
  ...
  const [recommandProduct, setRecommandProduct] = React.useState([
    products[0],
    products[1],
    products[2],
  ]);
  ...
  const header = () => {
    return (
      <View style={{width: '100%', position: 'absolute', zIndex: 9999}}>
        <SafeAreaView>
  ...
  return (
    <Layout hiddenAreaView={true}>
      ...
      >
        {header()}
        <>{item.images.length >= 1 ? <Slider images={item.images} /> : null}</>
        <View
          style={[
        ...
        >
          ...
          >
            ...
            <Text
              style={[
                AppStyle.style.h3,
                {color: theme.colors.active, fontWeight: 'bold'},
              ]}
            >
              ${item.price}
            </Text>
          ...
          <Text
            style={{fontSize: 13, marginBottom: width * 0.03, color: 'gray'}}
          >
            {item.description}
          </Text>
          <>
              ...
              <View style={{width: '50%', padding: 5}}>
                <TouchableOpacity
                  style={[
                    ...
                    Style.selectWidgetContainer,
                  ]}
                  onPress={() => setStateSelecter(0)}
                >
                  <Text
                    style={[
                      AppStyle.style.h5,
                      {
                        fontWeight: 'normal',
                        color: stateSelecter == 0 ? 'black' : 'gray',
                      },
                    ]}
                  >
                    SIZE
                ...
                >
                  <Text
                    style={[
                      ...
                      {
                        fontWeight: 'normal',
                        color: stateSelecter == 1 ? 'black' : 'gray',
                      },
                    ]}
                  >
                    COLOR
      ...
      >
        <View style={{flexDirection: 'row'}}>
          <View style={[Style.groupCountContainer, {marginRight: 15}]}>
            <IconButton
              style={Style.buttonCountContainer}
              icon={'minus'}
              onPress={() => {
