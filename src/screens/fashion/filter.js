...
export default ({ navigation, route }) => {
    ...
    return <Layout containerStyle={[AppStyle.style.marginTopLarge]}  >
        <ScrollView showsVerticalScrollIndicator={false}>
            ...
            <Dropdown
                containerStyle={[AppStyle.style.marginBottomLarge]}
                title="Sizes"
                footer={() => <ScrollView style={AppStyle.style.marginTop} showsHorizontalScrollIndicator={false} horizontal={true}>
                    {
                        sizes && sizes.map((item, index) => {
                            return <View key={`sizes-${item}-${index}`}>
                                <CheckBox
                                    label={item}
                                    visible={size == item}
                                    onPress={() => {
            ...
            <Dropdown
                ...
                title="Price"
                footer={() => {
                    return <View style={[{ flexDirection: "row" }, AppStyle.style.marginTopLarge]}>
                ...
                footer={() => <ScrollView style={AppStyle.style.marginTop} showsHorizontalScrollIndicator={false} horizontal={true}>
                    {
                        colors && colors.map((item, index) => {
                            return <View key={`colors-${item}-${index}`}>
                                <CheckBoxColor
                                    color={item}
                                    visible={color == item}
                                    onPress={() => {
