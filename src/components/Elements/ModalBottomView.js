...
export default ({
  ...
  const containerStyle = useAnimatedStyle(() => {
    ...
    return {
      transform: [{scale: containerHandle.value}],
      opacity: containerHandle.value,
  ...
  React.useEffect(() => {
    if (visible) {
      containerHandle.value = withTiming(1, {duration: 0}, finished => {
        if (finished) runOnJS(childAnimation)(1);
      });
    } else {
      containerChildHandle.value = withTiming(0, {duration: 400}, finished => {
        if (finished) runOnJS(containerAnimation)(0);
      });
    }
  }, [visible]);
