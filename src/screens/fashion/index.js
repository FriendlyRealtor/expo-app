...
export default ({navigation, route}) => {
  const [imageBanner, setImageBanner] = React.useState(
    images.backgroundWomenCollection,
  );
  const [products, setProducts] = React.useState(product);
  const header = React.useCallback(() => (
    <View style={{marginHorizontal: -width * 0.05}}>
      <Search
        chooseTopic={route.params.title}
        topicPress={title =>
          navigation.navigate(RouteName.FashionMenu, {title: title})
        }
      />
      <View style={AppStyle.style.marginHorizontalLarge}>
        <ImageBackground
          source={imageBanner}
          style={[AppStyle.style.imageContainer]}
        ...
        >
          <View style={AppStyle.style.imageContentContainer}>
            <Text
              style={[
                AppStyle.style.h5,
                {alignSelf: 'center', color: AppStyle.color.white},
              ]}
            >
  ...
  return (
    <Layout>
      ...
      <FloatButton
        ...
        icon={icons.filter}
        onPress={() => navigation.navigate(RouteName.Filter)}
      />
    </Layout>
  );
};
