...
export default ({ item, onDeleteCart = () => { }, onMinusCart = () => { }, onPlusCart = () => { } }) => {
    ...
    const close = () => {
        ...
        heightContainerHandle.value = withTiming(0, {duration: 300}, (finished) => {
            if(finished)
                runOnJS(onDeleteCart)();
        });
    }
    const rightButton = (progress, dragX) => (
        <Animated.View style={[Style.cartItemImageContainer, Style.rightButton, rightButtonstyle]}>
            <IconButton onPress={close} icon="close-circle" size={60} color={theme.colors.active} />
        </Animated.View>
    );

    return <Swipeable renderRightActions={rightButton}>
        <Animated.View style={[Style.cartItemContainer, containerStyle]}>
            ...
            <View style={{ flex: 1, paddingHorizontal: width * 0.03, marginTop: 15}}>
                <View style={{ flex: 1, flexDirection: "row", }}>
                    ...
                    {
                        item.size ? <Text style={{ marginLeft: width * 0.03 }}>{item.size}, </Text> : null
                    }
                    {
                ...
                <View style={{ flexDirection: "row", marginBottom: 15 }}>
                    ...
                    <View style={{ flexDirection: "row", borderWidth: 1, borderRadius: 5, alignItems: 'center' }}>
                        <IconButton icon={"minus"} size={16} onPress={onMinusCart} />
                        <Text style={{fontSize: 15}}>{item.count < 10 ? `0${item.count}` : item.count}</Text>
